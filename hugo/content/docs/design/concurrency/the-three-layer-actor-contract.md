...
The protocol layer is where the framework's position on shared mutable state becomes constructive. The coexponential types a stateful server serving an unbounded client pool, which is the shared-state pattern that motivates richer logics, and it does so inside classical linear logic, not its differential extension, Mix-free and with no sum-valued semantics, and no departure from the discipline the rest of the architecture holds. Placed at the actor boundary and inferred rather than annotated, it gives the framework a proof-carrying account of shared state that integrates with the tiers, rides the joint-constraint axis, and admits the client-arrival race a distributed server actually runs. The sum, where it is wanted at all, stays in the verification tiers as a distribution over the abelian carrier, not in the operational calculus. This is the design we will keep building toward as the actor runtime matures and the inference question comes into focus.
