...
This is the type theory of the stateful Olivier actor. An actor owns an arena, receives client requests serialized through its mailbox, and threads the arena state from one request to the next. Ownership identifies independently written state for the placement analysis described in [Counting the Cost of Coordination](/blog/counting-the-cost-of-coordination/). Physical cache isolation additionally requires suitable allocation alignment and extents; disjoint ownership alone does not prevent false sharing, and mailboxes and allocator metadata retain their own coordination costs. That is a server holding a pool of clients and evolving state across their interactions, which is what the coexponential types. The correspondence matters for two reasons. It gives the stateful-actor pattern a proof-theoretic account inside ordinary CLL, not the differential extension, Mix-free and with no sum-valued semantics. The client-serving race a real server runs is expressed as operational nondeterminism, the order in which the pool is served, and never as a formal sum. That fits our framework more closely than any account that pushes the race into a sum-valued denotation.
...
**Tier structure and mode shifts.** Structural protocol rules, where supported, remain Tier 1; a maintained counter range can be a Tier 2 arithmetic obligation. Tier 3 adds parameterized domain/system laws, including resource handoff and protocol preservation. A relation between client orderings or implementations may require Tier 4. [Mode shifts](/docs/internals/verification/mode-shifts/) retain the shared participants, premises and checked translations; an adjunction does not imply an invertible round trip. The [proof-composition design](/docs/internals/verification/proof-composition-and-tooling/) explains the Iris/Actris/Aneris and Verdi reuse opportunities. The compiler should elaborate supported instances automatically from the PSG. Their actual proof dependencies determine the trust boundary: a Rocq-founded Tier 3 law does not become solver-only when cvc5 checks its arithmetic premises.
...
The supported protocol rules need their own sound interpretation and bounded elaboration. Local invariants remain within their admitted analysis fragments; richer system or relational results use registered Tier 3 or Tier 4 rules through checked mode shifts. This does not establish a general decision procedure for distributed correctness or protocol inference.
