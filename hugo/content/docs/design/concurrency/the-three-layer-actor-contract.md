...
The protocol layer introduces no new verification axis. It is richer labeling on three structures the framework already carries: the joint-constraint axis, the graduated tiers, and the BAREWire wire format. The protocol layer reuses each in turn, and adds no machinery of its own.
...
The alternative the actor tradition has developed for the same liveness properties is model checking, and setting the wait-for rank against it shows what typing buys. McErlang, built for Erlang, the ancestor of our Olivier and Prospero discipline, verifies deadlock freedom and mutual exclusion by exhaustive execution: it drives a controllable runtime through every reachable interleaving and reports a proof or a counterexample. Its authors are precise about the cost. One search for a leader-election error took, in their words, "about 30 minutes, and explored somewhere around 10 million states," and the coarse-grained semantics they adopt to bound the search "means that some errors will go undetected which would be caught using a smaller-step semantics." Verification cost scales with the reachable state space, and coverage is traded against it by hand. Our wait-for rank asks the same liveness question of the same actor structure and answers it as a QF_LIA acyclicity obligation over the program's edges, discharged once at Tier 2, with no interleaving enumerated. The properties a model checker recovers by executing every schedule are, for the fragment the wait-for relation covers, entailments of a static graph the compiler already holds.
