...
description: "Ariel, the scheduler under Prospero, and its six-clause contract, from the credential unikernel's interrupt binding to hosted runtimes that only borrow fairness"
...
Our actor system has gone by two names for some time: Olivier and Prospero. Olivier is the actor machinery, and defines what an actor wraps, for lack of a better term. Prospero is a special form of supervisor, and decides what should happen and has a role in memory: restart strategies, arena lifecycles, placement and sentinels. The component that makes those decisions happen *in time*, the one that selects which ready actor runs next and delivers each resume, has until now been billed as part of Prospero. The billing for a hosted environment (such as an application running in a host operating system) was accurate as far as it went: modes of scheduling, how dispatch is provided per target, vary independently of anything the actor model assumes *outside* of the most common scenarios. On that recognition we moved the enabling/scheduling component to formal standing, with its own name, ***Ariel***, and its own contract in the specification: the [Scheduler Contract](/spec/draft/scheduler-contract/).
The one-sentence version of the constellation: Olivier defines what an actor *is and does*, Prospero decides how it should be orchestrated during runtime, and Ariel manages the machinery adjacent to both.

## Why the Scheduler Has a Distinct Name

Our compiler work already decides most of what a scheduler in other systems has to discover at runtime. [Region classification](/docs/design/concurrency/dcont-inet-duality/) determines what can run in parallel. Escape analysis determines where values live. The [wait classification](/spec/draft/synchronous-rpc-liveness/) determines which calls suspend and which do not. All of that is eligibility, and eligibility is derivable from program structure within the assumption of an operating system. Order under scarcity was the first narrowing question we encountered. Which ready actor runs next, what happens at saturation, whose delivery is admitted when capacity is tight: these decisions depend on runtime facts no compile-time analysis sees. This certainly can happen in a hosted environment but the considerations go further than that when considering embedded applications and unikernels.

For a while our working assumption was that the scheduler would fall out of that principled structure, and on the smallest target it nearly does. The [DCont representation](/spec/draft/dcont-representation/) establishes that on a single-core freestanding target the suspend/resume semantics become the cooperative scheduler, with resume as the scheduling event and the interrupt-to-continuation binding as dispatch. Obligations changed our position as our research went further into more specific application strata. Three factors now point at the need for a distinct scheduler:
...
A microVM profile assumes vCPU progress against a quota it can cite from the deployment specification. A container profile makes the same assumption about carrier-thread progress, against a budget it can only observe. At the hosted tier, fairness is inherited wholesale from the OS or managed runtime, with ordering and admission provided above it. The simulated profile assumes nothing and scripts everything.
...
- [Deadlock Freedom as an Obligation](/docs/design/concurrency/deadlock-freedom-as-an-obligation/) - The acyclicity discharge whose fairness premise this contract supplies
...
- [DCont Representation](/spec/draft/dcont-representation/) - Resume as the scheduling event, and the single-core reduction the scheduler contract adopts
