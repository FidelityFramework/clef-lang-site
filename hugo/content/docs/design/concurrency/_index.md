...
These articles trace how that foundation was chosen and how the compiler lowers async Clef code to native continuations without a managed runtime, from the divergence with .NET's async lineage to liveness across actor boundaries carried as a visible compile-time obligation. The same reading of a region's structure decides whether its concurrent strands merely reorder or cross in an order the runtime can observe, the distinction the braid work lifts into the type system.
