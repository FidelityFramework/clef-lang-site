...
flowchart LR
    ...
    App --> Signing["Signing Library<br>Static Binding"]
...
[dependencies]
# Hashing library - statically bound
hashing_lib = { version = "1.2.0", binding = "static" }
...
binding.overrides = { hashing_lib = "dynamic" }

[profiles.release]
# Release builds use static binding where possible
binding.default = "dynamic"
binding.overrides = { hashing_lib = "static" }
```
...
module Platform.Bindings.Hashing =
    /// int hash_compute(const void* data, size_t length, void* output)
    [<FidelityExtern("hashing", "hash_compute")>]
...
func.func private @hash_compute(%arg0: !llvm.ptr<i8>, %arg1: i64) -> !llvm.ptr<i8> attributes {
  llvm.linkage = #llvm.linkage<external>,
  fidelity.binding_strategy = "static",
  fidelity.library_name = "hashing_lib"
}
...
let secureBootSequence () =
    // Verify firmware signature (using a statically linked signing library)
    let firmwareHash = Signing.computeHash(FirmwareImage.data)
    let isValid = Signing.verifySignature(firmwareHash, FirmwareImage.signature)
...
```toml
[dependencies]
signing_lib = { version = "1.2.0", binding = "static" }
stm32l4_hal = { version = "2.1.5", binding = "dynamic" }
    ...
    | PlatformType.Mobile ->
        {
            ...
            PriorityStaticList = ["signing"; "core_algorithms"]  // Static
...
1. Use static binding for security-critical components (signing, authentication, authorization) to eliminate the risk of library substitution attacks.
