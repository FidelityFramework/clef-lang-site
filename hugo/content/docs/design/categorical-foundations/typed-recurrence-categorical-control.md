...
The CDL paper's adjoint correspondence (Fwd ⊣ Bwd) applies at each composition boundary. In λ-RLM, the forward pass through the combinator tree produces subproblem results; the Reduce operation is the "backward" composition that aggregates them. In the porous loop, the forward recurrence produces queries; the ADM response is the "backward" constraint that grounds the continuation. Neither paper makes this categorical connection explicit, but the structure is present in both.
