...
The [compilation sheaf design document](/docs/design/categorical-foundations/the-compilation-sheaf/) treats the four tiers as four sheaves over the same compilation poset, distinguished by their stalk categories. The staged-discharge architecture witnesses global sections of each sheaf, and the consequence rule applied at every lowering pass composes the verification across the pipeline.
