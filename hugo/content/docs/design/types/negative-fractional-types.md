...
A type usually describes a value the program produces. Two things a working program routinely does fall outside that description. It runs a step backward: an audit trail reconstructs the state before a decision, a search abandons a branch and returns to the choice point. And it carries a demand it has not yet met: a value that stands for evidence still to arrive, a slot a later step will fill. Ordinary type systems have no term for either one, so the program tracks them by hand, with stored snapshots for the reversal and ad-hoc plumbing for the deferred demand.

Negative and fractional types give each a first-class form. A **negative type** is a value flowing backward through evaluation, the type-level account of reversal and backtracking. A **fractional type** is a value that carries an unmet demand, the type-level account of constraint propagation. James and Sabry established these as the two dualities of computation in 2012, and Chen and Sabry later gave them a categorical interpretation in compact closed categories. In conventional languages they have stayed research artifacts, because their soundness asks the language to never silently duplicate or discard a value, and few languages make that promise.
