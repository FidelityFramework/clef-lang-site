...
The design spans a graded range of substrates between the CPU and FPGA poles, and each has a different relationship to non-IEEE representation, directed rounding, and exact accumulation. The capability gate mediates that range: selection runs against the set a target offers, `R(T) = { r : capability(T, r) ≠ unavailable }`, and proposes a representation only where it can be realized. A developer may also reach for b-posit and the quire deliberately, to obtain precision a target's native float path does not provide, accepting a representation that is emulated or synthesized rather than native. One source then resolves to the appropriate realization on each target, with the selected realization surfaced at design time.
