...
Our [DTS verification pipeline](/docs/internals/verification/decidability-sweet-spot/) assigns dimensional annotations to every numeric value and solves consistency constraints via cvc5 in the `QF_LIA` fragment. The solution is polynomial-time, complete, and principal. Every dimensionally consistent program can be typed without annotation. The inferred type is the most general. The constraint system is finite and the solution algorithm terminates. This verification happens in Alex, before the pipeline forks to target-specific backends.
...
The [verification internals](/docs/internals/verification/) describe the cryptographic release certificate generated by `clef build --release`. The PSG is locked. The assertions from every verification pass are aggregated into a global SMT problem. The solver verifies the system. The resulting witness is cryptographically hashed alongside the compiled binary into a `.proofcert` artifact.
