...
Clef's schema-directed narrowing is the technique LISP-family languages with static-typed refinements (Typed Racket most clearly) have refined for two decades. The tags JavaScript carries at runtime, property names and `typeof` results, are the handles our compiler-generated validators need. If JavaScript were genuinely untyped at runtime, the approach would not work. Because JavaScript's `Object` is structurally a tagged associative value in the LISP tradition, the mature LISP-family techniques for narrowing dynamic tagged data to static records transfer directly.
