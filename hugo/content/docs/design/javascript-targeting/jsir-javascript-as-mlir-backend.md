...
Clef's schema-directed narrowing is consequently not a new idea; it is the technique LISP-family languages with static-typed refinements (Typed Racket most clearly) have been refining for two decades. The tags that JavaScript carries at runtime, property names and `typeof` results, are exactly the handles our compiler-generated validators need. If JavaScript were genuinely untyped at runtime, the approach would not work. Because JavaScript's `Object` is structurally a tagged associative value in the LISP tradition, the mature LISP-family techniques for narrowing dynamic tagged data to static records transfer directly.

This is the shape of the erasure argument. Clef has no obj in the .NET sense (no universal root, no implicit boxing, no runtime reflection over arbitrary values) anywhere in the language. Clef has obj in the LISP sense only at the JavaScript/WASM interop boundary, through specific structured constructs (`JsValue`, `JsRef<'T>`, schema-directed narrowing). [At the FFI boundary with C](/spec/draft/ffi-boundary/#11-core-invariant), at the wire boundary with our BAREWire, and on every native target, the type system is closed. V8's tag system is what makes the narrowing work: the runtime carries the freight that the static erasure leaves behind.
...
There is a third emission-model concern specific to the actor target, and it is worth stating constructively because it determines how a Durable Object is embodied rather than merely how its boundary is checked. A Durable Object is not a `fetch` handler; it is a class, instantiated once per object ID, and many instances are live in the same V8 isolate at once. Each instance owns isolated state. That is the entire point of the primitive: `idFromName("a")` and `idFromName("b")` address two actors that must not see each other's memory.
...
The JSIR path embodies the instance model directly. A Clef actor lowers to a Durable Object class whose constructor instantiates that actor's state, and the per-instance state cell is what the generated class holds, not a global the script installs once. The actor lifecycle (construction with `(state, env)`, message dispatch through `fetch`, WebSocket and alarm hooks, `storage` access) is generated from the Clef actor definition and the BAREWire schema, the same way the narrowing validator is generated rather than hand-authored. Isolation is then a property of the generated class instances, the way it is for any hand-written JavaScript DO class, rather than something reconstructed by hand on top of a single shared script. This is the design intent; the actor-lifecycle generation is upcoming work in the JSIR back-end, and it is the natural extension of the same principle that produces the narrowing validators and the schema-derived serializers described above.
...
**Fable** took a different tactical route than js_of_ocaml, and the reason is architectural rather than preferential. Fable was built for F#, and F#'s post-compile representation is .NET CLR IL, an IR designed around C#'s semantics (reified generics, sealed class hierarchies, boxing rules, structural variance). By the time F# source has been lowered to IL, F# idioms (pipes, curried functions, discriminated unions as sealed types, pattern matching expanded into cascading type tests) have been translated into CLR-shaped constructs. A hypothetical IL-to-JavaScript compiler would be compiling the CLR, not F#. The F# source AST is the last representation where F# idioms are still visible as F# idioms, which is why Fable walks the source AST. The reason is not that AST-walking was preferred over IR-walking; F# has no analogue to the stable, ML-preserving bytecode that OCaml provides. Fable remains the right answer for F# for this reason, and it is not going anywhere. Our Composer's JSIR path is the js_of_ocaml-shaped approach applied to Clef, which, like OCaml, owns its post-frontend representation end-to-end.
