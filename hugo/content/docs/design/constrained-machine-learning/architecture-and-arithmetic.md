...
That separated-subspace target is the point of contact with ADM. The [ADM substrate](https://arxiv.org/abs/2603.18104), collected in [A Deeper Dive]({{< ref "/docs/guides/_index.md" >}}), enforces block-separated structure by construction: a block-diagonal generator has a block-diagonal exponential, and the off-block entries are provably zero by the grade type system. Ma's derived architecture reaches the same geometry but by a different process, as the attractor of an optimization rather than our method with Clifford algebra and a typed invariant. The [positional-encoding analysis]({{< ref "the-constellation" >}}) elsewhere in this section shows the two approaches meeting on one concrete subsystem; here the claim is the general one. It's the same target geometry with construction in our case and convergence in Ma's.
