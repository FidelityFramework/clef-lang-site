...
From our design perspective, the failure people call a context problem is a property of one recurrence, not a law of cognition. A single recurrent loop processing a long input has to fit every commitment it has made into one state vector, and as the sequence runs the early commitments decay against the later ones. That decay is [context rot]({{< ref "/docs/design/categorical-foundations/structured-recurrence" >}}), the degradation of early-step information across a long closed loop, defined in the recurrence entry. A direct token-based model exhibits the same decay for the same reason, and an entire industry has grown up to slow it.
