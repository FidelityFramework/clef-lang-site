...
> Part of the Constrained Machine Learning section, and the one that opens a
> frontier rather than closing a problem. Where other articles build and bound
> a language node, this one asks a different kind of question: what happens
> when the framework's most distinctive type-theoretic contribution, the
> negative and fractional types developed for the memory model, is turned on
...
The reversible-architecture literature builds models reversible by construction and trusts the construction. The framework does more: the state transition is typed so that its adjoint exists and the round trip is the identity, and that obligation is discharged through the verification machinery. When an earlier state is reconstructed, the reconstruction is proven to recover the exact state, not an approximation.
...
Internal: every other article in this section, the scaffold article especially; the memory model section for the lifetime and escape discipline and the reversible-allocation reasoning; the negative-and-fractional-types paper for the adjoint machinery; the negative-types-inference-checkpointing research note, of which this article is the site-facing form.

External: the reversible-residual-network and reversible-RNN literature; the gradient-checkpointing line on the storage-versus-recomputation tradeoff; the growing-memory recurrent-cache line that motivates the inference-recall problem.
