---
title: "From Standing Models to Built Ones: A Migration Path"
linkTitle: "A Migration Path"
description: "The same staged-substitution strategy the compiler uses for backends, applied to the model behind the interface: adapt what exists now, then fine-tune, distill, and build, sliding from rented capability toward owned, sovereign, structurally-compatible models"
...
> Eighth and final in the Constrained Machine Learning section. The earlier
> articles argued for a constellation of typed domain models with a porous
> language node, and built that node up from architecture and arithmetic to
> the type machinery that reaches back into it. This article is the adoption
> path: how an organization works with what exists now and migrates, rung by
> rung, toward the fully built model the section has been describing. The
> framing is the one the compiler already uses for its backends, and it is the
> reason the whole thing is practical rather than aspirational. Speculative in
> its details; the strategy is not.
...
The worked example, end to end, is the same regardless of variant. A goal arrives, the node proposes grammar-valid Clef, Composer elaborates it, and the elaborated program dispatches typed calls to the domain models, which answer with guarantees the language model could not have supplied:
...
An open, reproducible build methodology is the right vehicle. The OLMo approach, fully open training data, code, and checkpoints, is the model to follow, because a from-scratch model meant to be a sovereign asset must be reproducible and auditable by the organization that owns it. A model whose provenance is a signed record of data, recipe, and checkpoints, the version-record discipline the [building article]({{< ref "building-the-model" >}}) carries over from ADM, is an asset an organization can stand behind, which an opaque downloaded checkpoint is not.

This rung is where latency and velocity reach the regime the section has been arguing toward. The model is small because it is derived rather than over-parameterized, it runs on the b-posit substrate the rest of the framework uses, and it is trained by a forward-mode loop that makes adaptation cheap. Time to first useful token is local inference on a compact model that offloads aggressively to typed domain models. And the flywheel is fully closed: the built model is adapted by the same cheap forward-mode loop, distilled by the same verifiable signal, and improved alongside the typed domain models it routes to, with every part of the loop in-house. This is the velocity that justifies the build. A rented model gives capability; a built model gives a compounding asset, where each improvement to the domain models, the adapters, and the base reinforces the others.
...
Internal: the whole Constrained Machine Learning section, especially the [building]({{< ref "building-the-model" >}}), [architecture and arithmetic]({{< ref "architecture-and-arithmetic" >}}), and [forward-mode]({{< ref "forward-mode-and-adaptation" >}}) articles; the Fidelity.CloudEdge documentation for the Cloudflare bindings; the BAREWire documentation for the wire and shared-memory transports; the ADM pre-print for the version-record discipline.

External: the Cloudflare AI Gateway and Workers AI documentation; the OLMo open language-model methodology for the from-scratch build; the sequence-level and on-policy distillation literature; the local-inference engines (the llama.cpp family, vLLM) that serve the rung-one local variant.
