...
The practical reading is the census's reading one more time. The standard's deferral is not a defect; scheduling policy genuinely does not belong to an instruction set. It belongs to a concurrency model with a contract, and a program whose actors, priorities, and wait edges were settled at design time arrives at a threaded wasm host needing only what the proposal actually provides: memory, atomics, and spawn. The gap the standard left open is the part of the stack we never left to chance. What it would mean to walk into that gap with a runtime of our own, ground inventory and all, is [the next entry's subject](/docs/design/wasm-targeting/being-the-use-case/).
