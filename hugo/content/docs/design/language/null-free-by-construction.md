...
Clef removes the question by removing its subject. A [flat closure](/docs/design/memory/gaining-closure/) carries a function value and a separate capture environment, every capture field initialized at construction with its established value, shared deferred identity or original cell view. Initializing that entry does not force an unevaluated initializer. The function value names an implementation; no capture slot is empty. Code is not a pointer field inside the environment. A [discriminated union](/spec/draft/discriminated-union-representation/) is one of its declared cases and nothing else. There is no case for "no case." Where a program needs to model absence, it says so in the type, with `Option`, and absence becomes a case the compiler makes the reader handle rather than a runtime state a reference could hold undetected until it is dereferenced. The [option operations specification](/spec/draft/option-operations-representation/) gives that its representation.
