...
Every language that supports first-class functions faces the same question: where do captured variables live? Whether you're writing Rust closures that require explicit lifetime annotations, Go functions that capture by reference, or Python lambdas that use late binding, creating unexpected state sharing, you've encountered the closure problem. JavaScript developers know the classic loop-variable trap. C# developers have watched the compiler generate mountains of hidden classes. The syntax varies; the underlying tension is universal. Our approach to this problem in Fidelity draws from decades of compiler research, specifically the Standard ML tradition and the MLKit project's "flat closure" representation, to generate native code that is memory-safe, runtime-free, and [null-free by construction](/docs/design/language/null-free-by-construction/). That last property reaches further than safety: it is also what lets the same closure cross between substrates, a consequence worked out in [Null-Free by Construction](/docs/design/language/null-free-by-construction/).

When a function captures variables from its enclosing scope, those variables must live somewhere; they many times persist beyond the stack frame that created them. In managed runtime environments, this is straightforward: the garbage collector handles everything. In native compilation without a runtime, the question becomes an architectural one.
...
The academic literature presents two approaches to closure representation. Understanding both clarifies why we chose the path we did in Fidelity.
...
More importantly for our purposes in Fidelity, flat closures are "safe for space" in the formal sense defined by Shao and Appel. A closure holds references only to what it actually uses. When the closure becomes unreachable, all captured values become unreachable. There is no hidden retention of environment chains.
...
The MLKit approach influenced our design significantly. While our current implementation uses stack allocation and explicit region management rather than MLKit's full region inference, the flat closure representation enables future adoption of more involved region-based schemes. The architecture does not paint us into a corner.
...
This is what we are building toward in Fidelity: familiar Clef idioms at design time, native performance at runtime. The compiler handles the translation, and the developer writes the code they already know.
...
Getting the foundation right here is what lets the higher-order machinery sit on top of it without surprises, and that is where our attention turns next as the work continues.
