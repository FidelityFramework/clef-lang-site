...
A general editor treats the compiler as a black box behind a language-server protocol. It sees diagnostics and completions, and little else. The Fidelity Framework's whole design-time story asks for more than that. Obligations discharged as a program is elaborated, [surfaced in the editor at the point of writing](/blog/between-a-rocq-and-a-hard-case/). Escape and arena placement shown as navigable annotations. The [Program Semantic Graph](/docs/internals/pipeline/learning-to-walk/) treated as a live object a developer can watch transform through the pipeline. Those are not features a plugin bolts onto a general editor. They are the surface the framework already describes in [Formal Verification as Compilation Byproduct](/docs/design/categorical-foundations/formal-verification-compilation-byproduct/), and Atelier is where that surface becomes a place to work.

## The WREN Stack, self-hosting by design
...
Atelier's reason to exist is the framework-specific surface, the design-time experience the rest of this site develops, brought into one environment rather than scattered across a diaspora of third-party tools:
...
- **Debugging for the control flow the model is built on.** First-class support for the [delimited continuations](/docs/design/concurrency/delimited-continuations/) the framework's concurrency model rests on, the captures and resumptions a traditional stack-frame debugger would struggle to show.

Each of these is the presentation layer for work the compiler already does, and many editors would likely support a substantial portion of these features readily. Atelier does not invent the analysis; it makes the analysis legible, in one place, at the instant it matters. The goal here is time to insight and having a no-excuses design-time experience that feels native to the framework, as opposed to the plug-in bloat of other systems that leave the workflow feeling bolted-on after the fact.
