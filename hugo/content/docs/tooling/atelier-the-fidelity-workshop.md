...
Atelier is targeting the experimental [WREN Stack](/blog/wren-stack/): a reactive frontend in [Partas.Solid](https://github.com/speakeztech/Partas.Solid) rendered in the system WebView and a native backend compiled by [Composer](https://github.com/FidelityFramework/Composer). [WRENHello](https://github.com/FidelityFramework/WRENHello) supplies the working F#/Fable/Solid frontend and Composer-native Linux host; its current bridge uses shared command/event types with bounded text codecs over WebKit script messages. BAREWire/WebSocket integration remains proposed. The [UI review of 18 September 2026]({{< relref "/docs/design/user-interfaces" >}}) places WREN within a shared functional model alongside a new Clef-native reactive-area engine. These experiments inform Atelier's layout and ownership design without establishing a production multiwindow toolkit or measured cross-platform resource advantage.
