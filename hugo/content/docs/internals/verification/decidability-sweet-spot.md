...
The Dimensional Type System (DTS) is restricted to a specific algebraic niche that general dependent type systems cannot exploit.
...
The analogy is to regular expressions and context-free grammars. Regular expressions form their own formal class, with closure properties, recognition algorithms, and applications unlike those of the grammars above them. DTS occupies the analogous position relative to dependent types, a separate formal class that overlaps in expressive power for a specific domain but differs in the computational properties practical tooling depends on: decidability, inference completeness, and runtime cost.

The decidability holds because every quantity in the paragraphs above lives in a finitely-generated free abelian group, where the order of composition never changes the result and consistency reduces to comparing integer vectors. The check stays cheap because composition commutes, and that commutativity holds only inside a boundary. Concurrent work whose crossing order the runtime can observe does not commute, and the group governing its rearrangement is the non-abelian braid group \(B_n\), not \(\mathbb{Z}^n\). Ordinary code remains inside the abelian interior with no additional obligation. The abelian projection of a crossing, its writhe, stays a Tier 1 quantity decided by the same integer linear algebra, and only observable crossing structure carries a heavier obligation, one discharged by reusing proofs the dual-pass has already produced. [The braid as a fourth sheaf](/docs/design/categorical-foundations/braid-as-a-fourth-sheaf/) is the proposed, non-normative design for that boundary: the sweet spot is the abelian interior, and the non-abelian edge is something we are taking some care to treat appropriately.
...
## The Hoare Logic View

The mechanism above maps onto two standard constructions in program verification: weakest-precondition computation for the design-time pass, and Hoare's consequence rule for the compile-time re-discharge.
