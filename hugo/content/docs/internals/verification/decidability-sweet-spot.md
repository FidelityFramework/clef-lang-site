...
This decidability rests on one property: every quantity in the paragraphs above lives in a finitely-generated free abelian group, where the order of composition never changes the result and consistency reduces to comparing integer vectors. The check stays cheap because composition commutes, and that commutativity holds only inside a boundary. Concurrent work whose crossing order the runtime can observe does not commute, and the group governing its rearrangement is the non-abelian braid group \(B_n\), not \(\mathbb{Z}^n\). That boundary leaves ordinary code where it is. The abelian projection of a crossing, its writhe, stays a Tier 1 quantity decided by the same integer linear algebra, and only observable crossing structure carries a heavier obligation, one the design assembles from proofs already in hand. [The braid as a fourth sheaf](/docs/design/categorical-foundations/braid-as-a-fourth-sheaf/) is the proposed, non-normative design for that boundary: the sweet spot this article describes is the abelian interior, and the non-abelian edge is something we are taking some care to treat appropriately.
