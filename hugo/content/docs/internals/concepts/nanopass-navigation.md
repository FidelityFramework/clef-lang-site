...
Most compilers follow a familiar structure, as we touched on in our blog entry [Frontend Unfuzzled](/docs/internals/mlir/frontend-unfuzzled/). The "front end" parses developer source into an abstract syntax tree, often with type-related and other syntactic checks. The compiler's "middle end" performs transforms on intermediate representations, and the "back end" generates machine code. Managed runtime compilation, as practiced by .NET and Java, represents a different category altogether. Those systems target virtual machines with garbage collection and JIT compilation, abstracting to 'assemblies' handed over to a runtime to manage the workload.

Traditional ahead-of-time native compilation employs varied implementation strategies, though most established compilers share a common architecture: monolithic passes that recursively process entire program structures through intertwined transformations. The monolithic design began as a practical necessity. When memory was measured in kilobytes and disk space was precious, minimizing intermediate representations and performing multiple transformations in single traversals made economic sense. It evolved as standard practice for languages with stable semantics targeting mature platforms; a pattern that newer systems languages have largely adopted even though those original resource constraints no longer apply. While this architecture represents decades of hard-won engineering stability, it quickly reveals its limits when compiler infrastructure must adapt to emerging hardware architectures and novel execution models.

As discussed in [The Return of the Compiler](/blog/the-return-of-the-compiler/), we are witnessing a fundamental shift away from managed runtimes toward native compilation, as heterogeneous computing and specialized accelerators dominate the landscape. The current AI super-cycle is clear evidence of that shift.

Nanopass compilation inverts the traditional compiler approach. Instead of monolithic passes that interleave interdependent transformations in a single traversal, a nanopass compiler decomposes transformations into discrete, single-purpose passes that each perform one well-defined operation on an intermediate representation. The paradigm originated in Scheme compiler research and gained prominence through the [nanopass framework](https://github.com/nanopass/nanopass-framework-scheme) developed by Andrew Keep and R. Kent Dybvig. Their work demonstrated that compiler complexity could be managed through numerous small transformations rather than fewer large ones.

The Composer compiler applies this principle across the entire compilation pipeline, extending nanopass architecture past any single tier. From Clef source code to native binary, Composer currently executes approximately 25 distinct passes, each operating on precisely defined intermediate representations. We made this architectural choice in response to constraints encountered while designing for "multi-stack targeting".
...
Each pass maps one specified intermediate representation to another. Platform-specific logic concentrates in Phase 4c, which consumes platform parameters without threading them through recursive calls. The other phases remain portable across all targets.
...
The **BackEnd** delegates to mlir-opt for dialect lowering and LLVM for final code generation. Composer itself embeds no target-specific lowering strategy. In its current, early form, the compiler stays close to 'standard' portable MLIR using `memref`, `arith`, `func`, `index`, and `scf` dialects. Target-specific transformations occur in mlir-opt, which provides specialized lowering passes for diverse platforms.

## The Migration to Portable MLIR

Early Composer development hard-coded LLVM types directly into MiddleEnd MLIR generation. String types became `!llvm.struct<(ptr, i64)>`, pointers became `!llvm.ptr`, and memory operations used LLVM-specific intrinsics. The hard-coding worked for x86-64 but blocked multi-stack targeting.
...
This change propagated through the witness layer: Platform library functions needed updates to how they compose MLIR operations, and memory operations shifted from LLVM intrinsics to portable `memref` dialect operations. Target-specific mlir-opt passes now determine the final type representation.

## Lessons from Extant Art

The nanopass framework repository provides theoretical grounding for structuring a compiler as single-purpose passes. Keep and Dybvig formalized the notion of pass composition through language definitions that specify valid transformations. Their framework generates pattern matchers and traversal code from declarative language descriptions, ensuring each pass operates only on valid inputs and produces only valid outputs.
...
The [Triton-CPU project](https://github.com/triton-lang/triton-cpu) contributed practical patterns for MLIR-based compilation. Triton-CPU adapts the GPU-focused Triton compiler's progressive lowering approach to target CPUs instead, demonstrating that the same MLIR dialect architecture could serve different hardware substrates. Their work paralleled our research and showed that portable intermediate dialects could coexist with target-specific lowering passes. We had already arrived at this model in Composer's early design, with the goal of broader application across devices and architectures.

## Formal Foundations of Pass Composition

The practical side of nanopass compilation comes from implementation patterns like those in Triton-CPU. Targeting hardware from embedded microcontrollers to neuromorphic processors also depends on a formal account of transformation correctness, which reaches further than exhaustive testing can on its own.
...
Navigation in this design is both mechanical and strategic. In the mechanical sense, successive passes walk precisely defined intermediate representations, and the PSGZipper provides bidirectional traversal of the Program Semantic Graph so witnesses observe context without threading state. In the strategic sense, we are charting a course through a rapidly evolving hardware landscape.
...
Nanopass architecture addresses this variability by isolating target-specific decisions in late-stage lowering passes. Composer's FrontEnd and MiddleEnd produce portable MLIR that describes program semantics without committing to a representation for any particular target, and the BackEnd (mlir-opt and LLVM) provides the target-specific transformations. Adding a new target means implementing lowering passes without modifying the compiler core.

The architectural boundary occurs at dialect selection. [Portable dialects](/spec/draft/backend-lowering-architecture/#21-portable-dialects) (`memref`, `arith`, `func`, `index`, `scf`) describe operations common across targets. Target-specific dialects (`llvm`, `spir-v`, `hls`) encode platform assumptions. Composer emits only portable dialects, delegating specialization to mlir-opt. This division mirrors the distinction in formal language theory between context-free grammars (portable structure) and attribute grammars (context-specific annotations).
...
Composer builds on decades of programming language theory, adapted to the practical constraints of multi-stack systems programming. Its late-stage lowering is designed to admit neuromorphic arrays, optical switches, and quantum processors as targets.

This work is a concrete expression of the trajectory that [The Return of the Compiler](/blog/the-return-of-the-compiler/) described as inevitable for systems programming. The migration from LLVM-specific types to portable MLIR establishes a unified compilation pipeline where LLVM lowering continues to serve established architectures while the same nanopass framework extends to neuromorphic processors, reconfigurable fabrics, and emerging accelerators.
