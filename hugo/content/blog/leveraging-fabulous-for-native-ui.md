...
Fabulous provides a functional API for building user interfaces in F#, managing state through the Model-View-Update pattern, and updating the UI through its diffing algorithm. In our framework that state flow rests on a [push-based reactive substrate](/spec/draft/observable-computation/), carried in practice by [Fidelity.Rx]({{< ref "fidelityrx-native-reactivity" >}}). Our challenge would be to take these high-level patterns and compile them down to direct calls to LVGL (Light and Versatile Graphics Library) and Skia, removing runtime overhead in the process.
