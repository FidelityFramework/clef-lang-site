...
In our current designs, the first checkpoint is the end of the front end, when CCS has completed its work. After elaboration produces the saturated graph and before lowering begins, the relational property would be established on the source model. The transition from the decidable tiers up to the relational concern is the kind of typed coercion our [Mode Shifts](/docs/internals/verification/mode-shifts/) proposal sketches. At this point Rocq has whole program as written to that point, with the local facts it consumes already present on the graph. While this is considered as part of the design-time experience, we imagine it to be prely optional and only enabled as the domain demands.
