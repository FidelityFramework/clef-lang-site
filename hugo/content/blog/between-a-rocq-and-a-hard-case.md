...
A developer wants their program to be memory safe, the threads to never deadlock, and their buffers to never outlive their owner. Our design provides all three (and more) in concurrent Clef, with no annotation and no handwritten proof term to crowd the code. The compiler verifies those facts in the Program Semantic Graph (PSG) continuously and is re-verified at build time. There is another guarantee above that, the kind a cryptographer or a control engineer reaches for, and it is supported by a distinct but sympathetic mechanism. The discipline that keeps that level of proof machinery off the everyday developer's desk is the same discipline that makes the everyday guarantees *free*, which is why the more demanding tier is worth understanding on its own merits.
...
The rejection-sampling loop is the one hax could not verify, and the same loop is the one this framework verifies directly. It runs until enough candidates are accepted, acceptance depends only on the public candidate value, and the write index advances branchlessly so the loop's control flow carries no secret. That branchless structure is exactly what hax lost: its extraction turned the loop into a proof-inert form, so the verifier signed off without ever seeing whether a secret reached a branch. Here the loop is not extracted into a separate model. The branchless control flow is carried on the graph and re-checked on the lowered code, so the property the verifier confirms is a property of the instructions that run.
...
It moves development forward, because the obligations are discharged as the program is elaborated and surfaced in the editor at the point of writing. We want the most common proof terms to be a design-time element in order to support correctness, a property that identifies issues immediately instead of after a separate verification pass, a failed audit, or a production incident. The feedback loop that formal methods usually defer to the far end of a project sits at the moment of authorship instead. Our approach should also lighten the annotation burden, because the structural facts that higher obligations consume, the dimensions and lifetimes and escape behavior and grade, are inferred from the code's own structure rather than declared by hand. The annotations a conventional verified-programming workflow makes the developer write are, in the cases our algebra covers, derived by the compiler instead of requiring the developer to hand-roll it directly.
...
Most of what the compiler verifies is "local" to the code being expressed. The dimension of a value, the escape class of an allocation, the width of an integer: each is a property of a single point in the program, and each is discharged in a narrow decidable theory, integer-linear arithmetic or bit-vectors, fast enough to surface in the editor at the point of writing. [The Decidability Sweet Spot](/docs/internals/verification/decidability-sweet-spot/) is the account of why staying inside that fragment is what makes verification in our framework both seamless and computationally cheap, and [Formal Verification as Compilation Byproduct](/docs/design/categorical-foundations/formal-verification-compilation-byproduct/) is the wider tour of what the compiler hands the developer 'for free'.

By contrast, some properties are not local. Whether a set of actors can deadlock is a fact about how the whole program fits together, and no single operation can see the cycle, because the cycle is a relationship among many. The framework handles this, and it handles it without leaving the decidable fragment. As [Deadlock Freedom as an Obligation](/docs/design/concurrency/deadlock-freedom-as-an-obligation/) lays out, each synchronous call carries only its own wait-for edge as a local fact, the acyclicity obligation rides on the enclosing region, gathers every edge, and proves a ranking exists. The rank is an integer assigned to each actor so that every wait-for edge strictly increases it, which exists exactly when the relation is acyclic. That constraint, \(r(u) < r(v)\) for every edge, is integer-linear arithmetic, the same kind of obligation as an interval check, so it lowers into the SMT dialect and the solver discharges it like any other.
...
Each `let! … = callee.PostAndReply …` suspends the signer until the callee answers, so each is one wait-for edge with the callee at its head. Reading the saturated graph, the compiler collects those edges over the enclosing region and emits the acyclicity obligation. No edge is written by the developer; each falls out of a `PostAndReply` the code already contains.
...
So neither 'spanning' nor reaching into *the reals* steps away from proof terms the compiler discharges automatically. This commitment to staying inside the decidable fragment is something we arrived at on our own, but it is not without precedent. Dafny reached it first, by a different road. 
...
The difference in kind is also why the implementation and expected audience is narrow. A specialist reaches for a relational guarantee because the programmatic use case demands it. The design establishes the property in the domain's own terms, an EUF-CMA game, a constant-time requirement, and the compiler builds the derivation that discharges it. At this level of engineering, stating the property and building its proof are separate tasks.
...
The `relating` clause names the pair, which is the irreducibly relational element a value-refinement type cannot express. The `couples` clause specifies how the two runs' random choices are paired, the machinery that makes the distributional reasoning tractable. The `consumes` clauses name the lower-tier premises, so the dependency is explicit and the reconciliation can confirm those premises were actually discharged. The `dispatch_to` clause says where the obligation goes, so the tier boundary is admitted in the surface and never silently assumed. The specialist writes the `relating`, `requires`, `couples`, and `ensures` that state the judgment; the compiler fills the `consumes` from the obligations already on the graph and supplies the dispatch.

The premises arrive across the tier boundary as typed shifts, and the developer doesn't have to deal with any of them. There is no `shift` keyword in Clef. The shift is internal compiler machinery: Baker will read the saturated graph, recognizes from the loop's branchless structure that a Tier 2 fact would admit a Tier 3 termination claim, and emits the shift that carries the obligation. The termination fact climbs from Tier 2 to Tier 3 that way, and the relational judgment sits above both on a second shift the same elaboration emits, each one carrying the obligation that the lower tier's structure admits the higher tier's claim. What the developer sees is the `sampleUniform` loop from before, unchanged; the shifts are derived from it.

To emphasize the distinction from prior art, a Dafny `assert` cuts within one stratum and is placed by the developer at design-time, where their experience shows the solver will stall. The Clef shift crosses between strata and is placed by the compiler, from a structural signal in the code. The same cut that is manual in Dafny is derived *and **typed*** here. Our Tier 3 termination our PSG would discharge is itself a library-carried lemma, proved once and instantiated against the loop's concrete bound, so the loop does not re-derive it. That lemma is library code, written by whoever builds the cryptographic lemma library, not by the engineer writing the signing routine.
...
The framework spans the program routinely and stays in the decidable fragment while it does, and deadlock-freedom is the proof. The fragment widens into the reals as fractional and negative types require, and stays decidable. The one concern that exits is the relational, probabilistic one, and it exits on the probabilistic axis, because that is a different kind of logic, not because it spans and not because it reaches the reals. It lives at the proof kernel, established once at the end of the front end on the source model and designed to be re-established once after lowering on the emitted artifact, by the compiler as its final stage and re-runnable by anyone over the shipped binary. The lowering runs between the two, carrying the local facts the conditional consumes and nothing probabilistic itself, because the only proof vehicle the pipeline has is scoped to exactly the decidable fragment the framework targets.

The relational guarantee is a property of the binary rather than a model of it. The failures that only appear during compilation are caught where the information to catch them finally exists. The compiler stays small and the trusted base stays small, because the guarantee is re-confirmed on the output rather than carried through the machinery. And the developer did not write the proof by hand to collect any of it.

For the specialist, the signing routine is in a familiar domain, and the compiler derives the relational obligation, builds the proof terms, and re-checks them on the binary. The coupling proof that a high-assurance audit would otherwise demand by hand is the one piece the specialist is spared. For the generalist who rarely reaches this narrow passage, it is still worth seeing how the framework reasons about its furthest reach, because that same design is what shapes the everyday surface, with the lower proof layers generated automatically underneath.
