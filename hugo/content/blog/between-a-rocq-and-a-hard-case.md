...
A developer who wants their program to be memory safe, the threads to never deadlock, and their buffers to never outlive their owner gets all three from ordinary concurrent Clef, with no annotation and no handwritten proof term. The compiler verifies those facts from the code and checks them as the program is designed (and re-verified at build time). There is another guarantee above that, the kind a cryptographer or a control engineer reaches for, and it is supported by a different mechanism. The discipline that keeps that level of proof machinery off the everyday developer's desk is the same discipline that makes the everyday guarantees *free*, which is why the more demanding tier is worth understanding even for an engineer who may never invoke it.
