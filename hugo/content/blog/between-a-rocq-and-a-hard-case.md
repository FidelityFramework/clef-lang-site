...
Every developer wants their code to be memory safe, the threads and processes to never deadlock, and for no buffer to outlive its owner. Our design aims to resolve all three (and more) in concurrent Clef, with no annotation and no handwritten proof term to crowd the code. The language, the Program Semantic Graph (PSG), and the elaboration and saturation pipeline that builds it are in place; the proof layer that reads obligations off that graph and discharges them is the next work to commence in the compiler scaffold. By design Clef Compiler Services (CCS) traverses the graph continuously, at the point of writing. At build time, CCS carries what it has established through to a re-verification in MLIR, so those properties are preserved all the way to the output artifact. Above that sits a higher-order guarantee, the kind a cryptographer or a control engineer reaches for, resting on a distinct but sympathetic mechanism. We keep that heavier machinery off the everyday desk with the same care that makes a cryptographer's hard case tractable at all, which is why the upper tier rewards a read even from an engineer who never reaches it.
...
In our design, the compiler derives the security property, so the developer never writes the proof by hand to get its benefits. The framework reads the relational obligation off the routine's structure, builds the proof terms automatically, and re-checks them against the emitted binary, all on the developer's behalf. That posture, where the compiler carries the burden of proof and the developer writes the solution, is the one [Fearless Concurrency Gets Real](/blog/fearless-concurrency-gets-real/) develops for memory and liveness, here carried up to the most demanding tier the framework reaches.
...
That difference carries all the way to the top tier. The relational, probabilistic guarantee there is non-trivial, but we expect our design to be supportive of those furthest reaches: the local facts those "higher" proof tiers consume are automated for free, and the discharge and the artifact re-check are meant to be carried by the compiler. The developer's contribution is the domain encoding that had to be written anyway. Our automated proof elements at tiers 1, 2, and 3 do the structural work that lets a domain expert reach a relational guarantee without becoming a proof engineer, and that same machinery lets a generalist warrant memory safety and dimensional correctness without writing a single annotation.
...
How the lower-tier premises enter the audit artifact is, in our experience with these reviews, what a careful re-check turns on. A certification lab will re-run the proof in its own kernel. That re-check is only as strong as what the premises are: if the dimensional, lifetime, and constant-time facts were carried in *as **axioms***, the kernel would confirm a *conditional* rather than a fact, and `Print Assumptions` on the final theorem would list those axioms for anyone to see. So our design carries the premises in as ***proved*** *lemmas* instead. In our pipeline the linear-arithmetic premises are re-proved by the proof assistant's own tactics, and the bit-vector premises are reconstructed from the solver's certificate so the kernel re-verifies each step, which keeps the solver out of the trusted base. A party who re-runs that proof and inspects its assumptions would find only the assistant's base axioms and standard logic underneath. This is a clean result that labs would look for.

Our second checkpoint is also where a property that exists only in the artifact can be caught. A timing side channel, a secret-dependent branch the source never had but a compilation stage introduced, has no source-level fact to carry it, because at the source level there was nothing to carry. That fact exists only in the machine code, which is exactly where this checkpoint reaches: in our design it scans the emitted code for any secret-dependent branch or memory access that survived lowering, and feeds that result into the relational confirmation. The first checkpoint cannot see a hazard that did not exist yet at the source; the second is built to.
...
In proof-theoretic terms, our design reaches across the whole program without leaving the decidable fragment. The fragment widens into the reals as fractional and negative types require, and stays decidable. The one concern that exits is a narrower relational, probabilistic case. The lowering runs between the intrinsic and extrinsic proof obligations, carrying the local facts the conditional consumes.

The relational guarantee is meant to be a property of the binary. The compiler stays small and the trusted base stays small, because the design re-confirms the guarantee on the output. And the developer would not write the proof by hand to benefit from it.

For the specialist, the signing routine is in a familiar domain, and in our design Composer derives the relational obligation, builds the proof terms, and re-checks them on the binary. A hand-built coupling proof a high-assurance audit would otherwise demand is now a significantly reduced burden. For the generalist who rarely reaches for those domains, it is still worth seeing how the framework reasons about its furthest reach, because that same design is what shapes the everyday surface, with the lower proof layers generated automatically underneath to provide "negative-cost verification" for safety and efficiency in every application.

Rocq, the kernel formerly known as Coq, is where our "Tier 4" proof obligation is exercised, a recent decision in the framework's development. We chose it for the reason a certification lab would: it is the kernel under CompCert and the Verified Software Toolchain, a standard bearer that high-assurance engineering, research, and academic work already lean on, so a relational guarantee discharged in Rocq is one an auditor can re-check in a tool they understand. The other reason is closer to our own design. Rocq is an ML-family language descended from OCaml, the same lineage Clef occupies, so the kernel inhabits the family our framework already lives in. Its foundations sit in sympathy too: the constructive, type-theoretic ground Rocq stands on is adjacent to the ground our dimensional and grade algebra is built on, so the probabilistic tier reads as a continuation of the framework rather than a foreign appendage bolted to its top. The title of this post is a small joke on the name, while the choice behind it is quite straight-forward: when the hard case demands a kernel, it should be one the industry already relies on.
