...
Conclave targets edge infrastructure, with Cloudflare's Durable Objects as the current substrate beneath our own actor layer. We surveyed the platform's [actors framework](https://github.com/cloudflare/actors) as a candidate foundation and found ergonomics where Olivier requires semantics: named instances, per-instance SQLite storage, multiple alarms, and hibernation-aware WebSocket handling, with no one-way send, no mailbox, and no supervision anywhere in it. That scoping is deliberate on their part, and their README states the mission as making Durable Objects easier. So the actor semantics remain ours to carry on this target, and we mine that repository for platform lessons instead of binding to it.

The overlap with our own design exposition is structural. Our three-layer contract works through [a registry server](/docs/design/concurrency/the-three-layer-actor-contract/#a-worked-example-the-registry-server): one stateful actor threading a name table across a pool of clients, with `Register` as a fire-and-forget `Tell`, adding no wait edge, and `Lookup` as the one synchronous round trip the liveness layer ranks. Cloudflare runs that example as infrastructure. Name-to-actor resolution is the platform's addressing layer, and the actors framework carries a registry actor of its own, a tracking instance threading an instance table across every other actor's registrations. A Durable Object runs the same shape the example types: one server, its state threading across a serialized pool of clients, the client race carried as the order the platform serves requests. What the platform's registry lacks is the pair of contracts the worked example teaches. Its register leg is a dropped promise with the error swallowed, so admission has no verdict, and its lookup leg is a wait-for edge no rank ever checks. What Conclave adds is exactly those two contracts, on a shape the platform already runs. Where native actors run in OS processes, Conclave actors would exist as JavaScript objects within V8 isolates. The platform may evict these objects from memory during idle periods, a process called hibernation. When a message arrives for a hibernated actor, the platform instantiates a fresh object and rehydrates its state from durable storage. Hibernation is the transparent-activation model: a caller reads the same thing after a crash as after a sleep. We take up that distinction, and what it costs supervision, in [Surfacing the Scheduler](/blog/surfacing-the-scheduler/).
...
The sketch below records the runtime's job in that setting, interpreting `ActorEffect` values against platform facilities. Those facilities come from the Durable Object bindings our CloudEdge work already carries, and two disciplines govern them: messages cross as BAREWire frames on the socket layer, and platform RPC stays on the management plane for naming and instance lifecycle.
...
This model has a second root, contemporary with Erlang and seldom named beside it. Barbara Liskov's Argus, the distributed language she built after CLU, organized programs into guardians, and in her 2009 Turing Award lecture she described one as ["fundamentally an object,"](https://www.youtube.com/watch?v=GDVAHA0oyJU&t=3207s) one guardian to a machine, encapsulating its state behind an interface that was the only way to reach it. A file server was a guardian, and all access to its files went through it. That is the actor's encapsulation, named in a different decade. The recovery story is the part that most resembles OTP: guardians were restartable, and her account of the failure path is the supervision policy almost verbatim, "you fail the guardian, bring it back up in a clean state, and then you can go forward again." Argus paired this with transactions and checkpoint rollback, machinery our actors do not carry. What our Olivier and Conclave supervision inherits, through Erlang and Syme's `MailboxProcessor`, is the guardian's other half, an encapsulated unit that fails and restarts clean.
