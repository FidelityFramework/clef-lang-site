...
A CPU is not purely serial about this. Its SIMD vector units run one operation across a lane of elements at once, and a GPU's SIMT threads run in lockstep across their lanes, and both have exploited that kind of parallelism for decades. What they take is the regular, rectangular share: a fixed lane width and a uniform operation applied across it. The parallelism a program actually expresses is wider than the rectangle. Irregular reductions, independent work of varying shape, dependency structure that no fixed-width lane was built to hold. That residue runs in sequence on a control-flow target, one step after another, even after the vector units have taken their share. Flow loss is the measure of it, the data-flow parallelism a program holds that the hardware's own mechanisms leave on the table.

Most developers carry a version of this as a *hunch*. A loop feels like it *should* parallelize, a pipeline feels like it is leaving something on the table, and the feeling never becomes more than a feeling without a profiler and signifcant head-scratching after the fact. There is a great deal of that latent sense in working code, with no number attached to it. Flow loss analysis is the tool that makes that subjective ***objective***: our compiler has a design goal to embody the parallel structure rather than discarding it, so the gap between the source as written and what the CPU does would be a figure read off the graph at design time, before anything runs.
...
For a pure region, one whose [referential transparency](/docs/internals/concepts/seeking-referential-transparency/) means its operations carry no hidden ordering, the PSG exposes an interaction-net reduction potential where independent operations fire at once, and that is the ideal parallel form. The same region, lowered to a CPU by Alex, our Composer middle-end, becomes an instruction stream that visits those operations in an order, taking what its SIMD vector units can and sequencing the rest. Both describe the same computation. Flow loss is the gap, and the [DCont/Inet duality](/docs/design/concurrency/dcont-inet-duality/) is where that gap is meant to first surface as a figure the graph makes available rather than one a profiler reconstructs after a run.
...
Two ends of that bound matter here. A control-flow target runs the region as one stream, the \(p = 1\) case, where \(T_1 = W\): every operation in sequence. Add processors and the time falls toward the span \(S\), the floor Brent's bound cannot go below, and the speedup tops out at

\[
\frac{T_1}{T_\infty} = \frac{W}{S}
\]

the available parallelism. For the worked pipeline below, \(W = 3072\) and \(S = 3\), so \(W/S = 1024\): the region holds a thousand-fold parallelism, and a serial lowering takes none of it. Flow loss is that gap, reported as the ratio \(W/S\) a control-flow target forgoes, or equivalently as the fraction of work \(1 - S/W\) that could run concurrently. The bound and its two ends are Brent's. What is ours is reading \(W\) and \(S\) off the graph directly, where a control-flow toolchain has to reconstruct the dependency structure before either number is available at all.
