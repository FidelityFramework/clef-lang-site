...
The abstract pole does not escape this. It moves the burden rather than removing it. The same variance, in APL, is close to the mathematical definition and a pleasure to read:

```apl
var ← {(+/(⍵-(+/⍵)÷≢⍵)*2)÷≢⍵}   ⍝ mean of squared deviations
sd  ← weights var pressures        ⍝ ... but what are weights and pressures?
```

The first line is the whole computation: subtract the mean from each element, square, sum, divide by the count, with `≢` the tally and `+/` the sum across the array. Nothing names a width, a shape, or a length, which is the appeal. But the second line is where the burden reappears, because APL's conformance rules are checked at runtime, not before. If `pressures` is a 100-element vector the call is fine. If a refactor upstream made it a `10 × 10` matrix, the same glyphs still run, the reductions fold along the wrong axis, and `var` returns a ten-element vector that the next stage consumes as though it were a scalar through APL's silent scalar extension. No error is raised. The shape discipline that would have caught it was never written down anywhere the machine could check it. It lived in the developer's head, carried not just past the keyboard but all the way through interpretation, and surfaced as a plausible wrong answer one stage downstream. The concrete pole names too much too early. The abstract pole names too little and defers the reckoning to runtime. Both hand the developer a discipline to carry by hand.
...
The compiler can know the range, because it can read it off the structure: from the dataflow when the values are bounded by the arithmetic itself, or from a domain library that states the physics, or from an annotation the developer writes when they want to fix it by hand. Once it has the range, the choice is not a judgment call. It is a deterministic function: pick the representation that minimizes worst-case relative error across the interval.
...
Keep only the representations whose dynamic range contains the whole interval. An empty \(R_{\mathrm{cov}}\) is a reported error, not a silent fallback.
...
The developer's contribution stays with the code that had to be written anyway, the units the problem already demanded, and the representation falls out of it. This is the same bargain [fearless concurrency](/blog/fearless-concurrency-gets-real/) strikes for memory placement, where `let mutable x = 0` is ordinary syntax and the compiler classifies the escape and places the value, and the same one the [Rocq companion piece](/blog/between-a-rocq-and-a-hard-case/) strikes for proof, where the developer writes the routine and the compiler assembles the terms. The burden moves off the person and into the analysis. The developer writes intent; the compiler carries the consequence.

## A force, end to end

Take the simplest thing a developer can write here and follow it the whole way. A program needs the gravitational pull between two masses, so the developer writes it in the units the problem is stated in:

```fsharp
open Fidelity.Physics.OrbitalMechanics

let gravForce (m1: float<kg>) (m2: float<kg>) (r: float<m>) : float<N> =
    GravConst * m1 * m2 / (r * r)   // dimension N inferred for free
                                    // range SUPPLIED BY Tier 2: dataflow alone
                                    // cannot lower-bound r² in the denominator,
                                    // so the library states the physics range
```
...
The range is exactly what decides the representation, and on this expression the compiler cannot read it from the code alone. The denominator is `r * r`, and nothing in the dataflow puts a floor under `r`, so the interval analysis cannot bound `1 / r²` from the structure. This is where the domain library is meant to step in. `Fidelity.Physics` states the physics the developer would otherwise have to tabulate by hand, and from the gravitation law it supplies the range `[1e-11, 1e30]`, from the gravitational constant at the small end through stellar masses at the large.
...
What the developer did is the whole of their contribution. They wrote the masses and the radius in kilograms and meters, named the part the code was going to, and opened the orbital-mechanics module so the range had a source. What they got back, per target, read in the editor while typing, is the representation, the range that drove it, and how much accuracy each target preserves. The most consequential numerical decision in the program was a structural fact about the code the whole time, and it arrives as something to read rather than something to guess.
...
Naming `riscv64-posit` is the whole act. `Fidelity.Platform` reads the entry, hands the selector the offered set `R(T) = { r : capability(T, r) ≠ unavailable }`, and the editor stops floating the formats this part cannot run at all. Swap the line for `target = "x86_64"` and the same `float<newtons>` resolves to `f64` instead, because that host's table reports no posit hardware.
...
| `Quire.toPosit` at the end | the single rounding for the whole sum lands here, dimension checked: `newtons` in, `newtons` out |
...
That is the gift in deferring the inference. Holding the representation open is not a loss of rigor, it is what keeps the information the choice needs alive long enough to use it, and a target named late carries more of that information than any decision made at the door could have. We will keep building toward the day the real-valued pass stands beside the integer one it descends from, and the developer who wrote the units and named the part reads the rest, given back to them rather than asked of them.
