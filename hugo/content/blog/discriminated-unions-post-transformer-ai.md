...
The payoff shows up on every target. On a CPU, the "what case is this" question your .NET runtime asks on every access is already answered, so the generated code reads a known position instead of discriminating a value. Carry the same union to an FPGA and the layout describes the fabric directly; carry it over the wire and both ends were compiled from it, so nothing re-parses. On each substrate the program, at runtime, does *less* than the untyped version would. This is Stroustrup's negative cost of abstraction, one level up from a single machine: the structure the union carries feeds the layout the compiler chooses for each target, the same way, in our [negative-cost verification](/blog/between-a-rocq-and-a-hard-case/) story for our highest tier of formalism support, the facts a proof establishes feed the primitive representation and placement the compiler chooses. Structure that the compiler can read is structure no longer requiring a runtime host, and that holds across every substrate this work targets.

Two things make that resolution ours. The position the compiler resolves to lives in a [flat closure]({{< ref "/docs/design/memory/gaining-closure" >}}), so the shape is inlined rather than reached through a managed cell. And where a computation is deferred, the [thunk and its saturation]({{< ref "/docs/design/structure-and-performance/why-lazy-is-hard" >}}) are elaborated in by our compiler, ahead of time, rather than serviced by a host as the program runs. The DU-as-blueprint payoff follows from both: the compiler flattens and saturates the structure into the layout, so what reaches each substrate is already resolved. That "already resolved" is the same property that leaves no null state to check for, worked out under [Null-Free by Construction]({{< ref "/docs/design/language/null-free-by-construction" >}}).
