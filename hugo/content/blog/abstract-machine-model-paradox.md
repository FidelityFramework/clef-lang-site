...
> **Editor's note (September 2026).** This post reads the CMU delimited-continuation dialect and Martín Coll's interaction-net dialect as prior art that models the same abstract machines Clef's computation expressions express. Fidelity does not adopt either as a dialect: both models are settled as hypergraph structure on the Program Semantic Graph and witnessed as standard MLIR dialects, as [The DCont/Inet Duality](/docs/design/concurrency/dcont-inet-duality/#upward-migration) describes. The post is left as written otherwise.
...
[The DCont dialect, emerging from CMU research](https://github.com/CMUAbstract/mlir-wasm-dialect) — prior art, not a dependency — models exactly this: continuations as central to how modern programs execute. Every async operation, every suspendable computation, every cooperative multitasking system manipulates continuations to do its work. F#, the lineage Clef descends from, has carried this model in its async workflows for years, and that AMM aligns with MLIR's execution model.

The picture extends, again as prior art, with [Martin Coll's Inet dialect for MLIR](https://github.com/colltoaction/mlir-inet-dialect). Interaction nets provide a model of computation where parallel reduction is both possible and deterministic. Pure functions can be parallelized automatically without race conditions or synchronization overhead. Clef's expression-oriented, immutable-by-default style maps to this model. Our language's AMM does not force programmers to think about parallelism; it is structured so that parallelism emerges from normal code.
