...
And now for the first time we're putting a design-time safety net in a programming workflow that guarantees *if the compiler allows it, the eBPF program will not crash the OS kernel.* That's a bold claim, but one that we feel that we are singularly qualified to embrace.
