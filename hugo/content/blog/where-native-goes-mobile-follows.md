...
Every mobile platform Apple, Google, Samsung, and even boutique Linux distributions for handsets ship with a native binding surface. Android exposes the NDK with bionic libc, ELF shared objects, and direct JNI access to the kernel beneath the Java framework. iOS apps are Mach-O binaries linked against libSystem and Apple's frameworks; Swift and Objective-C are the conventional languages, but the binary format and ABI are open. Samsung's Tizen applications may be `.tpk`-packaged native ELF with full Wayland compositor access. And even Linux mobile framworks like postmarketOS expose a standard Linux toolchain and a Wayland session on hardware. The native binding surface is, on every modern mobile target, the substrate the platform vendor's own applications run on.

Most developers take those provided framework 'shortcuts'. The dominant cross-platform solutions (React Native, Flutter, Kotlin Multiplatform, .NET MAUI, Capacitor, Cordova) abstract the native binding surface behind a uniform runtime. The demands of higher performance mobile applications means that native compilation would eventually mature: Flutter added AOT compilation after the Dart VM was established; React Native added Hermes after the JavaScriptCore interpreter was already shipping; Kotlin Multiplatform added Kotlin/Native alongside Kotlin/JVM; .NET MAUI added NativeAOT to the existing Mono and CoreCLR runtimes. However, in these systems the native compilation arrives within the framework's abstractions already shaped around their runtime contract, and the native mode conforms to a given 'shape.'

Our Fidelity Framework was built from **the *other* direction**. Native compilation across heterogeneous platforms was the starting assumption, and the compiler architecture and binding tooling were chosen because they support producing native artifacts for arbitrary targets without a uniform runtime substrate underneath. Mobile, in this view, is one of the targets the native compilation path reaches, as opposed to a terraced implementation requiring a boutique toolkit. Platform-specific native binding surfaces are where mobile compilation lands.

What follows is the architectural outline of how that story will be shaped in the Fidelity Framework. Here we'll walk through the early design decisions that reveal a cross-platform native capability. This includes LLVM platform tuples as compilation substrate, two adjacent UI patterns our framework supports today and the per-platform packaging walkthrough. It complese with notes on the language-level mechanism that lets one signature satisfy six platforms, and the honest costs and sizable benefits the approach carries.
...
The language-level feature that operationalizes the cross-platform claim is signature conformance: a single Clef module signature can have distinct implementations, and the Composer pipeline selects the implementation matching the target tuple at compile time. The consuming code sees a uniform interface. The compiler resolves the platform-specific implementation, type-checks the conformance, and emits direct native calls. The [namespace and module signatures specification](https://clef-lang.com/spec/draft/namespace-and-module-signatures/#signature-conformance) defines the rules; the worked example with three platform-specific implementations appears later in this post.

### Memory model designed for arena and region allocation

Our memory model is free of garbage collection by design. Memory regions are explicit and lifetimes are inferred at compile time through Clef's coeffect tracking. ([The native memory management design](https://clef-lang.com/docs/design/memory/native-memory-management/) elaborates.) The consequence for mobile is direct: working-set memory tracks the application's actual data footprint, and the cold-start cost the user feels is bounded by the application's own initialization in conjunction with the hardware abstraction layer. There's no extraneous lag because there's no runtime bring-up underneath it.

### Coeffect tracking for resource invariants

Constant-time execution, allocation discipline, IO discipline, and syscall discipline are all coeffect properties expressible in Clef's compiler service and verified at design time through our coeffect-aware process ([Context-Aware Compilation](https://clef-lang.com/docs/internals/mlir/context-aware-compilation/) documents the mechanism). These properties are stated as preconditions, checked at compile time, and preserved through every nanopass to the emitted binary. Honest preservation requires that the binary executing on the device be the binary the verifier reasoned about; a JIT or an interpreter may reschedule, recompile, or interpret arbitrarily, and the executable footprint at run time may diverge from the one the verifier saw. Native compilation is the precondition that makes coeffect claims hold all the way to the loader. This is the operational link between the mobile compilation account in this post and the broader verification story in [Cryptographic Certainty](https://clef-lang.com/blog/cryptographic-certainty/ "Cryptographic Certainty"); on every Fidelity target, mobile included, coeffect-tracked properties reach binary.

Five choices, made together at the start. All five were in place before mobile compilation was a question. Each of them is non-trivial to retrofit into an existing runtime-first framework, which is why the conventional mobile frameworks took a different path - retrofitting AOT compilation on top of the existing runtime contract.
...
Three binary formats appear: ELF (Linux, Android, embedded), Mach-O (iOS, macOS), and bare ELF without dynamic linking (deep-embedded targets that link statically against the chosen runtime). Composer routes ELF and bare-ELF emission through LLVM's existing backend coverage. The iOS Mach-O path is designed to use a Swift-native-style compilation route so the resulting binary matches what Apple's own toolchain would produce, with libSystem, the Apple framework set, and the ABI conventions Apple's loader expects. Every artifact is in the platform's standard binary format; the platform's loader recognizes it without modification.
...
The signing model varies across platforms, and the binary inside each wrapper has the same shape as the binary the platform's own toolchain would produce. Our contribution is to compile to it natively, with the same dialect-aware pipeline that compiles to STM32 or to Linux desktop.

The architectural consequence is that, in our framework's view, the mobile targets sit next to the embedded targets on the same compilation infrastructure. They differ in the wrapper, the libc, the available UI substrate, and the signing model. They share Composer, MLIR, the nanopass pipeline, the module signatures, and the coeffect verifier; one compiler reaches every row in the matrix from substantially shared source.
...
The pattern maps across the platform matrix. On Linux desktop and on postmarketOS, WebKitGTK is the WebView host, available pre-built against both glibc and musl in standard repositories (Debian, Fedora, Alpine, postmarketOS); the host shim is a small OS process that instantiates a `WebKitWebView` instance and points it at the backend's localhost WebSocket. On Samsung TV with Tizen, the frontend ships as a `.wgt` web application running in Tizen's WebKit, and the backend ships as a separate `.tpk` Service Application; Tizen message ports carry our BAREWire payloads between them. On Android, Android System WebView (Chromium-based, distributed and updated by Google Play) hosts the frontend; the native backend runs as a foreground Service in the same APK; the IPC transport is AIDL or a local socket with BAREWire payloads. On iOS, `WKWebView` hosts the frontend, the native backend lives in the same application binary, and the IPC is via `WKScriptMessageHandler` carrying BAREWire payloads. iOS does not generally permit user-installable background services, so our BAREWire transport on iOS runs in-process and synchronously between the backend module and the WebView host.
...
Our wrapper would use a Swift `AppDelegate` and at least one `UIViewController`. For HelloWayland-pattern applications, the controller would host an `MTKView` and would forward lifecycle events to Fidelity-compiled callbacks. For WRENHello-pattern applications, the controller would host a `WKWebView` and register `WKScriptMessageHandler` instances that exchange BAREWire payloads with the in-process native backend. The Fidelity-compiled binary would be a Swift-native-compatible Mach-O for `arm64`, produced through a Swift-native-style compilation path so the artifact links cleanly against Apple's libSystem and the framework set declared in the project (UIKit, Metal, QuartzCore, Foundation, and so on) and lands on the device as a binary indistinguishable in shape from one Xcode would produce.

### Embedded (STM32, RA6M5)

The artifact in this case is simply a flashable image, produced by linking statically against the runtime the target tolerates and emitting raw ELF for the bootloader. The runtime selection is per-target: musl-static or picolibc for the libc surface, with vendor HAL bindings (CMSIS for Cortex-M cores, Renesas FSP for the RA family, ST's HAL for STM32) brought in through Farscape from each vendor's headers. The choice of libc and HAL is dictated by the flash budget, the peripheral set the application drives, and the runtime requirements ([Fidelity on STM32](https://clef-lang.com/docs/internals/hardware/fidelity-on-stm32/) walks the constraints). Embedded targets appear in the matrix because they share the compilation infrastructure with the mobile targets and differ in what they wrap around the binary; the LVGL UI strategy that runs on a microcontroller and the Metal UI strategy that runs on an iPhone are routed through the same nanopass lowering pipeline, parameterized by the target tuple.
...
The mobile compilation delineation above resides within the broader Fidelity Framework thesis as a major branch of its delivery substrates. Three implications connect them.

### Device fleet longevity

A Fidelity-compiled mobile application runs in smaller working memory where its React Native equivalent runs in 150 to 200 MB and its Flutter equivalent runs in 80 to 120 MB. For a productivity application, the difference between feeling responsive and feeling sluggish is exactly this difference. For a credentialing application, the difference between sub-second handshake completion and a five-second cold-start handshake is exactly this difference. Device fleet longevity, in the field, is dominated by working-memory pressure. Reducing that pressure, among other salient factors, extends the life of the install base.
...
Our coeffect-tracked properties (constant-time execution, allocation discipline, IO discipline, syscall discipline) reach binary on every Fidelity target, including mobile, because every Fidelity target produces a native artifact that the loader executes directly. The four-tier verification architecture from [Cryptographic Certainty](https://clef-lang.com/blog/cryptographic-certainty/ "Cryptographic Certainty") rides on the same substrate: Tier 1 structural correctness, Tier 2 range and inequality reasoning over QF_LIA and QF_BV, Tier 3 rejection-sampling termination through library-instantiated probabilistic lemmas, and Tier 4 relational reasoning checked against a Rocq-proved rule library. Every tier emits to the same Composer pipeline, and the pipeline emits to every row in the platform matrix. The mobile compilation story is one of the substrates our verification story rides; the substrates differ in wrapper and signing, and the verifier's reach is the same across them.
