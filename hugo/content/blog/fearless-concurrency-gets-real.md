...
Our compilation pipeline leans on this directly. Computation expressions decompose into delimited continuations for sequential effects and interaction nets for the pure parallel part, and it's the immutable, parametric core that lets the interaction-net side run symmetric and independent (the [DCont/INet duality doc](/docs/design/concurrency/dcont-inet-duality/) walks the decomposition).

The second piece is one surface for all of it. Async, actors, queries: in our language they're computation expressions that desugar through the same `Bind`, which is continuation capture under a different name (see [delimited continuations](/docs/design/concurrency/delimited-continuations/) for how the desugaring lands). You don't get a separate concurrency model bolted on beside the language. You get the language's own sequencing construct, reused.
...
For a value that flows through three functions, you annotate three signatures so each local check has what it needs. Most of the time the annotating is mechanical and you stop noticing. The fight starts where the annotations stop reaching: a struct that holds a reference and gets passed around, a sharing pattern the borrow checker cannot express in the lifetime vocabulary available, a relationship decided at runtime and never written in a type. At that point the static analysis has gone as far as it can, and something else has to take over from there.
...
The direction we are taking is to extend where the analysis can see, so the developer never has to annotate the gap. A value's lifetime, its sharing, and its wait-for relationships are facts about how the entire program fits together, so we represent them where that fit is visible: as edges in our [Program Hypergraph](/docs/design/structure-and-performance/coupling-and-cohesion/) that span functions and actors instead of living inside one frame. An analysis that reads from that graph can follow a value across the boundaries a local check can't follow, and derive the relationship a developer might otherwise have to spell out.
...
[A `PostAndReply` suspends the caller's continuation until the callee answers, and that suspension is a wait-for edge from caller to callee](/spec/draft/synchronous-rpc-liveness/#the-wait-for-relation). A fire-and-forget `Tell` posts to a mailbox and returns, adding no edge, so the asynchronous fraction of a program cannot deadlock through this at all. The plan is to collect the synchronous edges into a wait-for relation on our Program Hypergraph, on the joint-constraint axis that carries region and lifetime hyperedges. For the fragment where every callee is a statically resolvable actor reference, [deadlock freedom is acyclicity of that relation](https://arxiv.org/abs/2103.14466), and acyclicity is a rank function: an integer per actor behavior such that every edge `u -> v` has `r(u) < r(v)`, which exists exactly when the relation has no cycle. That constraint is QF_LIA, the same kind of Tier-2 obligation our interval and bound checks already use, which is what makes it a tractable thing to build rather than a research problem.
...
*Contract clarification, 26 September 2026:* this June article describes the intended cross-actor design. The guarded transfer and hoisting discussion below is not evidence that every actor or closure path has been implemented and validated. Closure completion requires the [four lifetime classes](/spec/draft/closure-representation/#33-escape-analysis), actual captured-storage identity, and checked target realization. A known callee or an enumerated capture list alone proves neither an actor transfer nor a covering lifetime; the actor protocol must establish its additional obligations.
...
Our escape-analysis design works the other way. The lifetime question lives in the program graph, so cross-boundary reasoning can retain the actual value and its storage relationships. `let mutable x = 0` remains ordinary syntax. Capture, return, and by-reference use contribute obligations; they do not select storage by themselves. The established lifetime selects stack, region, static, or permitted heap storage. There are no `'a`-tick lifetime annotations to thread, because the lifetime is a fact the compiler must establish. The earlier [managed mutability](/docs/design/language/managed-mutability/) report describes the implementation work that motivated this contract.

Consider a value that escapes from one actor up to an predecessor. The default keeps the value in its owning actor's arena and guards the escaped reference with a sentinel: one O(1) validity check at the boundary, with deterministic release when a counted obligation set empties, and no leak when references form a cycle. You add no annotation for any of that. From there our analyzer suggests hoisting the value into the ancestor's arena, which sheds the guard entirely. Hoisting is accept-to-optimize: the safe-but-guarded form is the pit-of-success default, the faster form is a visible suggestion you can take or override. The mechanics are in the [arena hoisting tooling doc](/docs/design/memory/arena-hoisting/) and further articulated in our [coeffect algebra](/docs/internals/verification/memory-coeffect-algebra/#arena-hoisting-across-the-actor-hierarchy) documentation.
...
The pieces we put together hold because they were chosen together. Computation expressions give one surface where async, actors, and queries all desugar through the same continuation capture, and the immutable, parametric core makes the parallel parts safe to run without anyone marking them. Don Syme brought `MailboxProcessor` into F# himself as a hat-tip to Erlang, a core-library type rather than a construct woven into the language's own semantics. We are taking that library actor and making it central to the semantics, the move laid out in [our unified actor architecture](/blog/unified-actor-architecture/), so concurrency is Clef's first concern, and the safety properties follow from that structure rather than being bolted on beside it.

The reason to make the actor central, rather than a library beside the language, is a boundary problem Barbara Liskov named precisely. Closing her 2009 Turing Award lecture, she pointed at how distributed programs get written: "you write your individual modules, but then when you want to connect them together, you're out of the programming language and sort of into this other world." A module carries the language's type discipline and its local reasoning; the connective tissue between modules drops into serialization and an unchecked runtime contract. Her conjecture was that ["maybe we need languages that are a little bit more complete now so that we can write the whole thing in the language."](https://www.youtube.com/watch?v=GDVAHA0oyJU&t=3047s) The ML family already answers a good part of that. Argus was among the first, and the typed-actor and session-types lines have carried it since, so an ML-family language is where Liskov would recognize her conjecture substantially answered. What we have not found elsewhere is the delimited continuation discipline held across a heterogeneous hardware substrate. A `PostAndReply` between two Olivier actors stays inside the language, the wait-for edge it creates is the same fact the deadlock analysis reads, and a message across the boundary travels through BAREWire as a structured contract both sides interpret by construction rather than a payload one side tags and the other parses. Our framework is positioned to keep that contract intact whether the two ends are threads on one host, processes across a network, or an actor on a CPU speaking to logic on an FPGA, where connecting the modules crosses a hardware boundary and not only a process one. The local reasoning Liskov spent CLU establishing inside a module is what we mean to carry across the boundary between them, on whatever the modules happen to run.

And more directly to our point here, in Clef we *intentionally* chose ***not*** to carry an `unsafe` keyword, because the boundary it would mark is a checked construct in our system. Deadlock freedom is designed to be proven where the wait-for graph is visible and would be supervised where live data picks the callee. For readers who want the formal treatment, the obligation framing and the rank-function discharge are worked out in the [fixed-point scaffolding pre-print](https://arxiv.org/abs/2606.02854), where the same structural integrity reaches down into the reals through our negative and fractional types. The fragment we will be able to prove grows as the program graph carries more of the lifetime and wait-for structure into the open, and that is the direction we keep building toward as the work continues. The decomposition this post leans on, sequential effects to delimited continuations and the pure parallel part to interaction nets, is one instance of a wider structural match set out in [Weaving the Braid](/blog/weaving-the-braid/): a compiler that parts the woven strands of a computation to lower each one, then rejoins them, so the crossings stay the compiler's burden and not the developer's.

## Small choices loom large

Rust started inside this same family. Its first compiler was written in OCaml, and early Rust carried more of the ML character along with it, a garbage collector, green threads, a typestate system, the runtime-assisted shape of an ML-family language. Mozilla's push to make it a viable C and C++ replacement is what traded those away for ownership and the borrow checker, and that turn is what made Rust the language it is, defined in reflection of C instead of descent from ML. It is a good language because of that choice. We chose a distinct path: keep the ML substrate and build on it something no language in this family has by default, concurrency that is safe and live by construction.
