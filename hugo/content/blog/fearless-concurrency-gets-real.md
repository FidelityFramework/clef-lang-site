...
The design we are building takes a unique direction, the compiler tracking to the shape of the code. A program describes the structure the problem calls for, the analysis follows each element as it flows, and the compiler carries the burden of proving the result safe and live. Our aim is to deliver a language where correctness and ergonomics are not traded against each other. That is what fearless concurrency should mean, and it is the direction Clef is being built to reach.
