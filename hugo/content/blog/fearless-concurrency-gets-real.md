...
The direction we are taking is to extend where the analysis can see, so the developer never has to annotate the gap. A value's lifetime, its sharing, and its wait-for relationships are facts about how the entire program fits together, so we represent them where that fit is visible: as edges in our [Program Hypergraph](https://speakez.tech/blog/coupling-and-cohesion/) that span functions and actors instead of living inside one frame. An analysis that reads from that graph can follow a value across the boundaries a local check can't follow, and derive the relationship a developer might otherwise have to spell out.
